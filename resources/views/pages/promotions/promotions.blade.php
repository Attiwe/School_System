                            ...
                            <div class="table-responsive">
                                ...
                                <div class="card-body">
                                    <form action="{{route('promotion.store')}}" method="POST">
                                        @csrf
                                        <dir class="text-danger "> المرحلة الدراسية القديمه </dir>
                                        <div class="row">
                                        ...
                                        <dir class="text-danger "> المرحلة الدراسية الجديده </dir>
                                        <div class="row">

                                            <div class="col-md-3">
                                                <label for="new_grade_id" class="form-label">المرحلة الدراسية</label>
                                                <select id="new_grade_id" name="new_grade_id" class="form-control"  >
                                                    <option value="" disabled selected>المرحلة الدراسية</option>
                                                    @foreach ($Grades as $grade)
                                                        <option value="{{ $grade->id }}" {{ old('grade_id') == $grade->id ? 'selected' : '' }}>
                                                            {{ $grade->name }}
                                            ...
                                            <div class="col-md-3">
                                                <label for="class_id" class="form-label">الصف</label>
                                                <select name="new_class_id" id="class_id" class="form-control"  >
                                                    <option value="" selected disabled>-- اختر الصف --</option>
                                                </select>
                                                @error('class_id')
                                                    <small class="text-danger">{{ $message }}</small>
                                            ...
                                            <div class="col-md-3">
                                                <label for="section_id" class="form-label">القسم</label>
                                                <select id="new_section_id" name="new_section_id" class="form-control"  >
                                                    <option value="">اختر القسم -- </option>
                                                </select>
                                            ...
                                            <div class="col-12 col-md-3 mb-3">
                                                <label for="new_academic_year" class="form-label">السنة الدراسية</label>
                                                <select id="new_academic_year" name="new_academic_year" class="form-control"
                                                    required>
                                                    <option value="">اختر السنة الدراسية</option>
                                                    @php
                                                        $current_year = date("Y");
                                                    @endphp
                                                    @for($year = $current_year; $year <= $current_year + 1; $year++)
                                                        <option value="{{ $year }}" {{ old('new_academic_year') == $year ? 'selected' : '' }}>{{ $year }}</option>
                                                    @endfor
                                                </select>
            ...
            if (grade_id) {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $('select[name="new_class_id"]').append('<option value="" selected disabled>-- اختر الصف --</option>');
                        $.each(data, function (key, value) {
            ...
            if (class_id) {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $('select[name="new_section_id"]').append('<option value="" selected disabled>-- اختر القسم --</option>');
                        $.each(data, function (key, value) {
